...
4. When ACP proposes a budget, the mission pauses in `AWAITING_FUNDING_APPROVAL`. The exact job, amount, and currency are stored only after **Approve this one-time ACP payment**. The approval is scoped to that provider job and amount, is consumed after funding, and cannot authorize a subscription or an automatic recurring charge. Without that durable approval, `JobSession.fund()` is unreachable.
